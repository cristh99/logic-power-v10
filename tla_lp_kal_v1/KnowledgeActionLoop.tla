...
DoingRequiresGates ==
  (actionState = "DOING") =>
    (authorized /\ preflight /\ live /\ mandate)

DoneRequiresEvidence ==
  (actionState = "DONE") => evidence
...
MandateRequiresAuthorizedExecutableWork ==
  mandate =>
    (authorized /\ actionState \in {"ASAP", "DOING", "DONE"})
...
KnowledgeCannotExecute ==
  (Scenario = "KnowledgeExecute") =>
    (actionState # "DOING" /\ status # "PASS")

CreateStartsWithoutCommitment ==
  (verdict = "CREAR") =>
    (actionState = "SOMEDAY_MAYBE" /\ ~authorized /\ ~live)

KillEndsInTrash ==
  (verdict = "MATAR") => (actionState = "TRASH")

PassHasFeedback ==
  (status = "PASS") => (verdict # "NONE")

KnowledgeToActionRequiresConsequence ==
  (Scenario = "MoveExisting" /\
   actionState \in {"ASAP", "DOING", "DONE"}) => consequence

ActionToKnowledgeRequiresEvidence ==
  integrated => evidence

FutureDateCannotIssueMandateEarly ==
  (Scenario = "FutureDate") => ~mandate
